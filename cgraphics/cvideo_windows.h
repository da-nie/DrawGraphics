#ifndef C_VIDEO_WINDOWS_H
#define C_VIDEO_WINDOWS_H

//****************************************************************************************************
//класс рисования с помощью Windows
//****************************************************************************************************
...
class CVideo_Windows:public IVideo
{
 private:
  //-переменные-----------------------------------------------------------------------------------------
  uint32_t ScreenWidth;//ширина экрана
  uint32_t ScreenHeight;//высота экрана
  CWnd *cWnd_Ptr;//указатель на класс окна

  HCURSOR hCursor_Magnify;//курсор увеличения
  HCURSOR hCursor_Cross;//курсор перекрестья

  CDC cDC_ScreenMemCopy;//копия внеэкранного контекста
  CBitmap cBitmap_ScreenMemCopy;//картинка копии внеэкранного контекста

  CDC cDC_ScreenMem;//внеэкранный контекст
  CBitmap cBitmap_ScreenMem;//картинка внеэкранного контекста

  LOGFONT LogFont;//тип шрифта
 public:
  //-конструктор----------------------------------------------------------------------------------------
  CVideo_Windows(uint32_t width,uint32_t height,CWnd *cWnd_Ptr_Set);
  //-деструктор-----------------------------------------------------------------------------------------
  ~CVideo_Windows();
 public:
  //-открытые функции-----------------------------------------------------------------------------------
